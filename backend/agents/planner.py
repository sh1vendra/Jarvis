...
def build_planner_agent() -> LlmAgent:
    ...
    return LlmAgent(
        ...
        "A MULTI-FIELD WEB FORM (e.g. a flight search with an origin, a "
        "destination, a trip type, and a date) needs one milestone PER FIELD, "
        "not one milestone covering all of them together - a real, live test "
        "found the Action agent exhausting its attempts finding one stubborn "
        "field (e.g. the origin) and never even trying the others in the same "
        "turn, silently leaving them unset while the milestone still reported "
        "its own outcome. Each field is itself a genuine, independently "
        "inspectable state - 'the destination shows New York' can be true or "
        "false regardless of what the origin field shows - so give each one "
        "its own milestone: e.g. 'the origin is set to Austin', 'the "
        "destination is set to New York', 'the trip type is set to one-way', "
        "'the departure date is set to next Friday', THEN a final 'the search "
        "is submitted' milestone (requires_approval=true). Skip a milestone "
        "entirely for a field that's already correct and needs no change "
        "(e.g. the trip type already defaults to what's wanted).\n\n"
        ...
        "A KAYAK FLIGHT SEARCH task specifically gets one more milestone, right "
        "after 'the flight search is submitted' (the requires_approval=true "
        "one): a final milestone that the top flight results are read back and "
        "ready to present to the user, e.g. 'the top flight results are read and "
        "presented' - requires_approval=false, since reading and describing "
        "results spends nothing and commits to nothing. This is the last "
        "milestone in a flight-search plan; there is no booking/purchase step "
        "yet, so never add one.\n\n"
